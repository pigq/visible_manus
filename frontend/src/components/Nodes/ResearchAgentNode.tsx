import { Search, Globe, MousePointer, Type, ScrollText, FileText, Loader2, CheckCircle2, XCircle } from 'lucide-react';
...
const toolIcons: Record<string, React.ReactNode> = {
  ...
  get_text: <FileText size={14} />,
};

function ToolCallItem({ toolCall }: { toolCall: ToolCall }) {
  const icon = toolIcons[toolCall.tool] || <Search size={14} />;
...
export function ResearchAgentNode() {
  const { researchAgent } = useStore();
  const { status, toolCalls } = researchAgent;
  ...
  return (
    <div className={`w-80 h-[300px] p-5 rounded-2xl border bg-zinc-900 flex flex-col shadow-xl transition-all duration-500 ${isActive ? 'border-orange-500 shadow-orange-500/20' : status === 'complete' ? 'border-green-500 shadow-green-500/20' : 'border-zinc-800'}`}>
      <div className="flex items-center gap-2 border-b border-zinc-800 pb-3 mb-3">
        ...
        <span className="text-sm font-bold text-zinc-300">RESEARCH AGENT</span>
        {isActive && (
          <motion.div
